...
class TemplateFile:
    ...
    class ModelInput:
        ...
        def add_inputs_from_instructions(self, instructions: list[Instruction], context_lines: int):
            ...
            unique_sets = {i: dict() for i in range(context_lines + 1)}
            for instruction in instructions:
                for idx, token_set in enumerate(instruction.get_token_sets()):
                    ...
                    # Create usage format
                    if idx == len(unique_sets) - 1 and token_set.is_user:
                        # If it's the last set and contains a user token, mark it as a prompt
                        token_keys += "\n<prompt>"
                    else:
                        token_keys += "\n<string>"
                    unique_sets[idx][token_strings] = token_keys
