...
class Instruction(BaseInstruction):
    """
    A Instruction is an instruction without a user prompt.
    ...
    def __init__(self, context: Sequence[TokenSet], response: TokenSet, final: Token = NON_TOKEN):
        ...
        if self.contains_user():
            raise ValueError(
                "Instruction requires that the response does not contain a UserToken. Use UnsetInstruction for user inputs.")
