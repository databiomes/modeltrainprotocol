class Instruction:
    ...
    def number_check(self, numbers):
        ...
        num_tokens = [[token.emoji] for tokens in self.tokens for token in tokens if token.num]
        ...
        for num, tok in zip(numbers, self.tokens):
            num_token = [token.emoji for token in tok if token.num]
            num = [num for num in num if num or num == 0]
            assert len(num) == len(num_token), "Not all numbers provided."

    def __str__(self):
        tokens_str = ', '.join([''.join([token.emoji for token in token_tuple]) for token_tuple in self.tokens])
        samples_str = ',\n'.join([f"Sample(Strings: {sample['strings']}, Result: {sample['result'].emoji + sample['value'] if sample['value'] is not None else ''}" for sample in self.samples])
        return f"Token Set(Tokens: {tokens_str}, Result: {self.result.emoji}, Samples:\n{samples_str})"
