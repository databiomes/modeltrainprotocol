...
class TestSimpleInstruction:
    ...
    def test_simple_tokenset(self):
        ...
        instruction: Instruction = Instruction(input=context, output=SIMPLE_TOKENSET, final=TOKEN_CONTINUE)

        assert len(instruction.context) == 2
        assert instruction.output == SIMPLE_TOKENSET
    ...
    def test_user_tokenset(self):
        ...
        instruction = Instruction(input=context, output=USER_TOKENSET, final=TOKEN_CONTINUE)
